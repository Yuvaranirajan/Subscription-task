...
const LoginPage = () => {
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] flex items-center justify-center p-6 bg-slate-50/50 dark:bg-slate-950 flex flex-col transition-colors duration-300 relative overflow-hidden">
      <div className="absolute top-0 -left-4 w-72 h-72 bg-emerald-100 dark:bg-emerald-900/20 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-blob" />
      <div className="absolute bottom-0 -right-4 w-72 h-72 bg-teal-100 dark:bg-teal-900/20 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-blob animation-delay-2000" />
      ...
      <div className="max-w-md w-full relative">
        <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-3xl shadow-2xl shadow-emerald-900/5 p-12 border border-white/20 dark:border-white/5">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl md:text-4xl font-extrabold text-slate-900 dark:text-white tracking-tight">Welcome Back</h1>
            <p className="text-slate-500 dark:text-slate-400 mt-3 text-base md:text-lg font-medium">Log in to manage your subscription</p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-emerald-600 text-white cursor-pointer py-4 rounded-2xl font-bold text-lg hover:bg-emerald-700 hover:shadow-lg hover:shadow-emerald-200 dark:hover:shadow-none transition-all active:scale-[0.98] flex items-center justify-center gap-3 group disabled:opacity-70 disabled:cursor-not-allowed"
            >
              {isLoading ? (
          ...
          <p className="text-center mt-10 text-slate-500 dark:text-slate-400 font-medium">
            ...
            <Link to="/register" className="text-emerald-600 dark:text-emerald-500 font-extrabold hover:text-emerald-700 dark:hover:text-emerald-400 transition-colors underline-offset-4 hover:underline">
              Create account
            </Link>
          </p>
        </div>
